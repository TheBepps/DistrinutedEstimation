...
mu_acc = 0.05; % proprioceptive sensors
sigma_acc = 0.5;
H = eye(2);
...
pos_meas(1,2:3) = pos_store(1,1,2:3) + randn(1)*sigma_pos_ext + mu_pos_ext; % first x measurement
pos_est(1,2:3) = pos_meas(1,2:3);
pos_meas(2,2:3) = pos_store(1,2,2:3) + randn(1)*sigma_pos_ext + mu_pos_ext; % first y measurement
...
for t=3:length(time)-2
    ...
    acc_meas(1,t+1) = (vel_store(1,1,t+1)-vel_store(1,1,t))/dt + randn(1)*sigma_acc + mu_acc; % real acc + measurement error
    acc_meas(2,t+1) = (vel_store(1,2,t+1)-vel_store(1,2,t))/dt + randn(1)*sigma_acc + mu_acc; % real acc + measurement error
    % First integration
    vx_prop = runge_kutta_4(acc_meas(1,t), acc_meas(1,t+1), dt, v_meas(1,t));
    vy_prop = runge_kutta_4(acc_meas(2,t), acc_meas(2,t+1), dt, v_meas(2,t));
    % Velocity estimation fusing v_bar and v resulting from
    % integrated acc
    v_meas(1,t+1) = vx_prop/(accuracy_ratio*accuracy_ratio + 1) + vx_bar/(1 + 1/(accuracy_ratio*accuracy_ratio));
    v_meas(2,t+1) = vy_prop/(accuracy_ratio*accuracy_ratio + 1) + vy_bar/(1 + 1/(accuracy_ratio*accuracy_ratio));
    % Second integration
    ...
    if (ext_meas(1) == 1 || ext_meas(2) == 1) && (flag_leader_close_to_anchor(1) == 0 && flag_leader_close_to_anchor(2) == 0) % only followers are close to the anchor
        ...
        x_meas = x_meas_prop(1)/(accuracy_ratio*accuracy_ratio + 1) + x_meas_ext/(1 + 1/(accuracy_ratio*accuracy_ratio));
        % y
        y_meas_ext = pos_store(1,2,t+1) + randn(1)*sigma_pos_ext + mu_pos_ext;
        y_meas = y_meas_prop(1)/(accuracy_ratio*accuracy_ratio + 1) + y_meas_ext/(1 + 1/(accuracy_ratio*accuracy_ratio));
    end
    ...
    if norm(now-goal) < 1
        ...
        if anchor_num < length(myMap.anchors_x)+1
            ...
            acc_meas(:,1:t) = 0; % integral starting from 0 (just keep the last acc meas to calculate the next step)
            v_meas(:,1:t) = 0; % integral starting from 0
            mu_acc = 0;
        end
    end    
