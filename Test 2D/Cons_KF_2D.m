...
pos_store(:,:,2) = p_0; % starting from time=2
vel_store(:,:,2) = v_0; % starting from time=2
...
sigma_pos = 2;
drift = 0.01; % drift (mu_pos increases at each step)
H = eye(2);
...
pos_meas(1,2) = pos_store(1,1,2) + randn(1)*sigma_pos + mu_pos; % first x measurement
pos_est(1,2) = pos_meas(1,2);
pos_meas(2,2) = pos_store(1,2,2) + randn(1)*sigma_pos + mu_pos; % first y measurement
...
P_store(:,:,2) = P_init; % Initialization
...
for t=2:length(time)-1
    ...
    if(distance>1 && abs(vx)<=0.08 && abs(vy)<=0.08)
        ...
        out = speedLimit([vel_store(1,1,t) vel_store(1,2,t)],[vx vy],dt,detect_R);
    end
    ...
    % accurately its position. Proprioceptive and exteroceptive
    % measurements are fused together to provide a better estimation.
    if ext_meas == 1
        % Sensor fusion
        % x
        x_meas_prop = x_meas;
        x_meas_ext = pos_store(1,1,t+1) + randn(1)*sigma_pos_ext + mu_pos_ext;
        accuracy_ratio = sigma_pos/sigma_pos_ext;
    ...
    % Set ext_meas = 0 for the next step
    ext_meas = 0;
    ...
    for i=2:n  % starts from 2, leader is out      
        ...
        for j=1:n       % Linear Consensus control
            ...
            if A(i,j)==1
                ...
                sum_edge_weight = sum_edge_weight + q_ij;
            end
        end
        ...
        vel_store(i,1,t+1) = K0*vel_store(1,1,t) + gamma*distance*cos(th);    
        vel_store(i,2,t+1) = K0*vel_store(1,2,t) + gamma*distance*sin(th);
       ...
       % Consider repulsion among submarines and store their pose to obs_pose
        ...
        out=speedLimit([vel_store(i,1,t) vel_store(i,2,t)],[vel_store(i,1,t+1) vel_store(i,2,t+1)],dt,detect_R);
    ...
    if norm(now-goal)<3
        ext_meas = 1; % Exteroceptive sensors activated (leader can see the anchor)
        mu_pos = 0; % When leader sees the anchor, proprioceptive drift is set to 0
    end
