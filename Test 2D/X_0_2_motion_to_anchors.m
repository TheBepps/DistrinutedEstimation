...
num_steps = 50;
duration = 0.001; % Adjust as needed

% Iterate through each anchor point
for i = 1:length(anchors_x)
    % Determine the angle between the initial orientation and the direction towards the current anchor point
    direction_theta = atan2(interp1(t, profile, anchors_x(i)) - triangle_dynamics(2), anchors_x(i) - triangle_dynamics(1));
    ...
    step_size_x = (anchors_x(i) - triangle_dynamics(1)) / num_steps;
    step_size_y = (interp1(t, profile, anchors_x(i)) - triangle_dynamics(2) + anchor_offest) / num_steps;
