...
myMap.anchors_x = [5, 20, 40, 60, 75, 91];                  % Declare the anchor array
myMap.anchors_y = [5, 25, 35, 50, 80, 91];
...
% Q_cov = Q_cov'*Q_cov;
% disp(Q_cov)
Q_cov = [0.026, 0.031, 0.033; 
         0.031, 0.035, 0.038; 
         0.033, 0.038, 0.040]; % error on system input to account for other disturbances
...
for t=3:length(time)-2
    ...
    yaw = atan2(goal(2) - pos_store(1, 2, t), goal(1) - pos_store(1, 1, t));   % arctan(y/x)
    pitch = acos((goal(3) - pos_store(1, 3, t)) / norm(goal - pos_store(1,:,t)));
    ...
    repulsion = computeRepulsion3D([pos_store(1,1,t), pos_store(1,2,t), pos_store(1,3,t)], ob_pose(1:(length(ob_seabed_short) + length(ob_temp)), :), detect_R);
    % Optimize seabed repulsion objects only in the location of the agents
    ob_seabed_short=ob_seabed((max(floor(min(min(pos_store(:,1,t)/2)))-5,1))*50:(min(floor(max(max(pos_store(:,1,t)/2)))+5,50))*50,:);
    ...
    for i = 2:n  % starts from 2, leader is out
        ...
        ob_pose = [ob_temp; ob_seabed_short; obs_pose];
    ...
    view(45,45); % azimuth angle, elevation angle
...
for i=1:n
    plot3(squeeze(pos_store(i,1,2:t)),squeeze(pos_store(i,2,2:t)),squeeze(pos_store(i,3,2:t)),color(1,i),'LineWidth',2); % plot path
end
for i=2:n
    plot3(squeeze(pos_store(i,1,2)),squeeze(pos_store(i,2,2)),squeeze(pos_store(i,3,2)),'bp','color',color(1,i),'LineWidth',1);    % plot followers' starting positions
    plot3(squeeze(pos_store(i,1,t)),squeeze(pos_store(i,2,t)),squeeze(pos_store(i,3,t)),'m^','color',color(1,i),'LineWidth',2);    % plot followers' final positions
end
plot3(squeeze(pos_store(1,1,2)),squeeze(pos_store(1,2,2)),squeeze(pos_store(1,3,2)),'*','color',color(1,1),'LineWidth',1); % plot leader's starting point
plot3(squeeze(pos_store(1,1,t)),squeeze(pos_store(1,2,t)),squeeze(pos_store(1,3,t)),'o','color',color(1,1),'LineWidth',2); % plot leader's final point

ob_pose=[ob_temp; obs_pose];
if size(ob_pose)~=[0 0]
    plot3(ob_pose(:,1),ob_pose(:,2),ob_pose(:,3),'Xk','LineWidth',2); % plot obstacles
end
grid on;
xlabel('x');
ylabel('y');
zlabel('z');
legend('Leader','Follower 1','Follower 2','Follower 3','Follower 4','Follower 5');
xlabel('x(m)');
ylabel('y(m)');
zlabel('z(m)')
title('Trajectories');
